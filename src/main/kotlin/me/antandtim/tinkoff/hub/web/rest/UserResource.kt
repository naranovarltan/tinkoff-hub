    ...
    fun createUser(@Valid @RequestBody userDTO: UserDTO): ResponseEntity<User> {
        ...
        if (userDTO.id != null) {
            ...
            return ResponseEntity.created(URI("/api/users/" + newUser.login))
                .headers(HeaderUtil.createAlert(applicationName, "A user is created with identifier $newUser.login", newUser.login))
                .body(newUser)
        }
    }
